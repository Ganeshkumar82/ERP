...
router.post(
  ...
  async function (req, res, next) {
    try {
      res.json(await vendor.AddUpdateVendorResponse(req, res));
    } catch (er) {
      console.log(`Error adding the vendor -> ${er}`);
      next(er);
    }
...
router.post(
  ...
  async function (req, res, next) {
    try {
      res.json(await vendor.UpdateVendorDetails(req, res));
    } catch (er) {
      console.log(`Error fetching vendor field -> ${er}`);
      next(er);
    }
  }
);

router.post("/getvendorfield", async function (req, res, next) {
  try {
    res.json(await vendor.GetVendorField(req.body));
  } catch (er) {
    console.log(`Error fetching vendor field -> ${er}`);
    next(er);
  }
...
router.post("/addcustompo", async function (req, res, next) {
  try {
    res.json(await vendor.addCustomPO(req, res, next));
  } catch (er) {
    console.log(`Error while adding the custom PO : ${er}`);
    next(er);
  }
});
