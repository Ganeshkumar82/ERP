  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      transporter.sendMail(mailOptions, function (error, info) {
        if (error) {
          console.error(
            "Error sending vendor quotation approval email:",
            error
          );
          resolve(false);
        } else {
          console.log(
            "Vendor quotation approval message sent: " + info.response
          );
          resolve(true);
        }
      });
...
async function sendVendorPO(
  ...
  try {
    ...
    console.log(`error sending vendor PO mail -> ${er}`);
    return false;
  }
}
