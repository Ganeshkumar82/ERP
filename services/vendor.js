...
//         );
//       }
//       secret = rfqData.STOKEN.substring(0, 16);
//       // Validate session token length
//       if (rfqData.STOKEN.length > 50 || rfqData.STOKEN.length < 30) {
//         return helper.getErrorResponse(
//           false,
...
<<<<<<< HEAD
      ...
      if (adminDetails.length > 0) {
        ...
        adminEmail = "ceo@sporadasecure.com";
      }
...
<<<<<<< HEAD
    ...
    if (!querydata.hasOwnProperty("senddate")) {
      return helper.getErrorResponse(
        false,
        "error",
        "Send date missing. Please provide the Send date.",
        "SHARE FORM LINK",
        secret
      );
    }
    const [sql] = await db.spcall(
      `CALL SP_INSERT_VENDOR_FORM_LINK_REQUEST(?, ?, ?, ?, ?, ?, ?, ?, ?,@vvendorid); select @vvendorid;`,
      [
        querydata.phoneno,
        querydata.emailid,
        querydata.vendorname,
        querydata.messagetype,
        `${apiserver}/vendoronboard`,
        querydata.messagetype,
        querydata.messagetype,
        userid,
        querydata.feedback,
    ...
    const vendorid = objectvalue1["@vvendorid"];
    if (vendorid != null) {
      try {
        const sql45 = await db.query(
          `INSERT INTO vendor_autoupdate(vendorreg_id) VALUES(?)`,
          [vendorid]
        );
      } catch (er) {
        console.log(er);
      ...
      // Set default messagetype to 1 (email only) if not provided
      const messagetype = querydata.messagetype || 1;
      const sql2 = await db.query(`select secret from apikey where status = 1`);
      var apikey = "15b97956-b296-11";
      if (sql2.length > 0) {
      ...
      try {
        const formLink = `${apiserver}/vendoronboard?STOKEN=${apikey}&vendorRegId=${vendorid}`;
...
async function UpdateVendorDetails(req, res) {
  ...
  try {
    ...
    if (querydata.hasOwnProperty("vendorId") == false) {
      return helper.getErrorResponse(
        false,
        "error",
        "Vendor id missing. Please provide the vendor id",
    ...
    try {
      ...
      if (isUpdate) {
        ...
        // Build dynamic update query - only update fields that are provided
        if (querydata.field == "vendor_name") {
          updateFields.push("vendor_name = ?");
          updateValues.push(querydata.vendor_name);
        }

        if (querydata.field == "address") {
          updateFields.push("address = ?");
          updateValues.push(querydata.address || null);
        }

        if (querydata.field == "state") {
          updateFields.push("state = ?");
          updateValues.push(querydata.state || null);
        }

        if (querydata.field == "pincode") {
          updateFields.push("pincode = ?");
          updateValues.push(querydata.pincode || null);
        }

        if (querydata.field == "contact_person_name") {
          updateFields.push("contact_person_name = ?");
          updateValues.push(querydata.contact_person_name || null);
        }

        if (querydata.field == "contact_person_designation") {
          updateFields.push("contact_person_designation = ?");
          updateValues.push(querydata.contact_person_designation || null);
        }

        if (querydata.field == "contact_person_email") {
          updateFields.push("contact_person_phone = ?");
          updateValues.push(querydata.contact_person_phone);
        }
        if (querydata.field == "email") {
          updateFields.push("email = ?");
          updateValues.push(querydata.email);
        }
        if (querydata.field == "business_type") {
          updateFields.push("business_type = ?");
          updateValues.push(querydata.business_type);
        }
        if (querydata.field == "year_of_establishment") {
          updateFields.push("year_of_establishment = ?");
          updateValues.push(querydata.year_of_establishment);
        }
        if (querydata.field == "gst_number") {
          updateFields.push("gst_number = ?");
          updateValues.push(querydata.gst_number);
        }
        if (querydata.field == "pan_number") {
          updateFields.push("pan_number = ?");
          updateValues.push(querydata.pan_number);
        }
        if (querydata.field == "annual_turnover") {
          updateFields.push("annual_turnover = ?");
          updateValues.push(querydata.annual_turnover);
        }

        if (querydata.field == "products_services") {
          updateFields.push("products_services = ?");
          updateValues.push(querydata.products_services);
        }
        if (querydata.field == "hsn_sac_code") {
          updateFields.push("hsn_sac_code = ?");
          updateValues.push(querydata.hsn_sac_code);
        }

        if (querydata.field == "description") {
          updateFields.push("description = ?");
          updateValues.push(querydata.description || null);
        }

        if (querydata.field == "bank_name") {
          updateFields.push("bank_name = ?");
          updateValues.push(querydata.bank_name);
        }

        if (querydata.field == "branch_name") {
          updateFields.push("branch_name = ?");
          updateValues.push(querydata.branch_name);
        }

        if (querydata.field == "account_number") {
          updateFields.push("account_number = ?");
          updateValues.push(querydata.account_number);
        }

        if (querydata.field == "ifsc_code") {
          updateFields.push("ifsc_code = ?");
          updateValues.push(querydata.ifsc_code);
        }

        if (querydata.field == u) {
          updateFields.push("iso_certification = ?");
          updateValues.push(querydata.iso_certification || null);
        }
        ...
        const updateSql = `UPDATE vendor_autoupdate SET ${updateFields.join(
          ", "
        )} WHERE vendorreg_id = ?`;
...
async function GetVendorField(vendor) {
  try {
    if (vendor && vendor.STOKEN && vendor.STOKEN.length >= 16) {
      secret = vendor.STOKEN.substring(0, 16);
    }
    ...
    if (querydata.hasOwnProperty("vendorid") == false) {
      return helper.getErrorResponse(
        ...
        "error",
        "Vendor id missing. Please provide the vendor id",
        "GET THE VENDOR FIELD",
        secret
      );
    }

    const sql = await db.query(
      `select * from vendor_autoupdate where vendorid = ?`,
      [querydata.vendorid]
    );
    if (sql.length == 0) {
      return helper.getErrorResponse(
        false,
        "error",
        "Vendor not found. Please provide the valid vendor id",
        "GET THE VENDOR FIELD",
        secret
      );
    } else {
...
async function addCustomPO(req, res) {
  try {
    var secret, sales, querydata, userid;
    try {
      await uploadFile.uploadCustomPO(req, res);
      sales = req.body;
      // Check if the session token exists
      if (!sales.STOKEN) {
        return helper.getErrorResponse(
    ...
    const requiredFields = [
      ...
      { field: "pogenid", message: "Generated PO ID missing." },
      { field: "date", message: "Invoice date missing." },
      { field: "po_amount", message: "Purchase order amount missing." },
      { field: "messagetype", message: "Message type missing." },
    ...
    const [sql1] = await db.spcall(
      `CALL SP_ADD_VENDOR_CUSTOM_PDF(?,?,?,?,?,?,?,?,?,?,?,?,?,@customid); SELECT @customid;`,
      [
    ...
    if (!invoiceid) {
      return helper.getErrorResponse(
        false,
        "error",
        "Error while adding the custom PO.",
        "ADD CUSTOM INVOICE",
        secret
      );
    }
    ...
    if (querydata.messagetype === 1) {
      // Send only email
      EmailSent = await mailer.sendCustompo(
        querydata.clientaddressname,
        querydata.emailid,
        "Your Purchase order from Sporada Secure India Private Limited",
        "custompopdf.html",
        ``,
        "CUSTOMPO_PDF_SEND",
        req.file.path,
